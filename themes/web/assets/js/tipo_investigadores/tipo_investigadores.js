...
function avisoEliminar(id) {
    Swal.fire({
        title: "¿Eliminar Tipo de Investigador?",
        showCancelButton: true,
        confirmButtonText: "Sí",
        cancelButtonText: 'No'
    }).then((result) => {
        if (result.isConfirmed) {
            $.request('tipoInvestigadorComponent::onEliminar', {
                data: { id: id },
...
function cargarFormulario(data) {
    const { tipo_investigador } = data;

    modoModificar();

    document.querySelector('#nombre').value = tipo_investigador.nombre;
    document.querySelector('#id').value = tipo_investigador.id;
}
...
function modoModificar() {
    ...
    titulo.textContent = 'Modificar Tipo de Investigador';
...
function modoRegistrar() {
    ...
    titulo.textContent = 'Registrar Tipo de Investigador';
