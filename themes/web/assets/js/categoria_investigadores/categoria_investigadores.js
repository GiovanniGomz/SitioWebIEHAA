...
function avisoEliminar(id) {
    Swal.fire({
        title: "¿Eliminar Categoria de Investigador?",
        showCancelButton: true,
        confirmButtonText: "Sí",
        cancelButtonText: 'No'
    }).then((result) => {
        if (result.isConfirmed) {
            $.request('categoriaInvestigadorComponent::onEliminar', {
                data: { id: id },
...
function cargarFormulario(data) {
    const { categoria_investigador } = data;

    modoModificar();

    document.querySelector('#nombre').value = categoria_investigador.nombre;
    document.querySelector('#id').value = categoria_investigador.id;
}
...
function modoModificar() {
    ...
    titulo.textContent = 'Modificar Categoria de Investigador';
...
function modoRegistrar() {
    ...
    titulo.textContent = 'Registrar Categoria de Investigador';
