...
class AnaquelComponent extends ComponentBase
{
    ...
    public function generarPDF()
    {
        ...
        \Log::info(json_encode($anaqueles));

        $titulo = 'Listado de Anaqueles de Estante #' . $estante->codigo;
        ...
        $pdf = Pdf::loadView('iehaa.anaquel::reporte', $data);

        return $pdf->download('reporte_anaquel.pdf');
    }
    ...
    public function generarExcel()
    {
        ...
        $url = get('id');
        $estante = Estante::where('url', $url)->first();
        $titulo = 'Anaquel del estante #' . $estante->codigo . ' IEHAA';
        ...
        foreach ($anaqueles as $anaquel) {
            ...
            $sheet->setCellValue(
                'B' . $fila,
                'Anaquel ' . $anaquel['codigo'] ?? 'No disponible'
            );
